...
public class StyleTemplate {
    ...
    public String guidanceFor(String style) {
        ...
        return switch (s) {
            ...
            default -> "剧集风格（电视剧/网剧，默认主流形态）：面向长篇连续改编。"
                    + "对白充分、人物关系清晰；剧情按自然节拍推进，重要节点制造悬念，"
                    + "便于后续按「集」编排（每集独立成章并留钩子）。";
        };
    }

    /** 是否为剧集形态（需要分集编排）。默认即剧集；电影/话剧/短剧/分镜为不分集的单本形态。 */
    public boolean isSeries(String style) {
        String s = style == null ? "" : style.strip();
        return !(s.equals("电影") || s.equals("话剧") || s.equals("短剧") || s.equals("分镜"));
    }
}
